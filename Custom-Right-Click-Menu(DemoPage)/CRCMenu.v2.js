...
const createRightClickMenu = () => {
  ...
  // 注册自定义分组
  menu.registerGroup({ id: 'general', name: 'General', order: 10 });
  menu.registerGroup({ id: 'edit', name: 'Edit', order: 20 });
  menu.registerGroup({ id: 'link', name: 'Links', order: 30 });
  menu.registerGroup({ id: 'image', name: 'Images', order: 40 });
  menu.registerGroup({ id: 'other', name: 'Other', order: 50 });
  ...
  */
  ...
  if (!document.body.contains(menu)) { document.body.appendChild(menu) } menu.mount(); return menu
}; if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', createRightClickMenu) } else { createRightClickMenu() }